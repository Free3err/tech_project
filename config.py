...
PERSON_DETECTION_CLUSTER_DISTANCE = 0.15  # Меньше для маленькой комнаты

# Минимальный размер кластера для человека (метры)
PERSON_MIN_CLUSTER_SIZE = 0.15  # Меньше для обнаружения на близком расстоянии

# Максимальный размер кластера для человека (метры)
PERSON_MAX_CLUSTER_SIZE = 0.5  # Меньше для маленькой комнаты
...
LIDAR_MAX_RANGE = 2.5  # Ограничено размером комнаты

# Минимальная дальность LiDAR (метры)
LIDAR_MIN_RANGE = 0.05  # 5см минимум
